...
export default function StudentProfile({ navigation, route }) {
    ...
    useEffect(() => {
        const id = route.params.params.student.id
        ...
        if (re.test(value) && passin == user.password) {
            ...
            reauthenticate(passin).then(() => {
                ...
                user.updateEmail(value).then(() => {
                    firebase.database().ref('Student/' + user.uid).update({ email: value })
                    onCancle();
                    setLoading(false)
                }).catch((error) => {
                    setLoading(false)
        ...
        if (value != undefined && value.length >= 6 && passin == user.password) {
            ...
            reauthenticate(passin).then(() => {
                ...
                user.updatePassword(value).then(() => {
                    firebase.database().ref('Student/' + user.uid).update({ password: value })
                    onCancle();
                    setLoading(false)
    ...
    const uploadUserProfilePicture = async () => {
        setLoading(true)
        const ref = storage().ref('Profile/Student/' + user.id);
        var uri = value[0].uri;
        ...
        firebase.database().ref('Student/' + user.id).update({ profilePicture: downloadLink }).then(() => {
            onCancle();
            setLoading(false)
        })
    }
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={{ flex: 8 }}>
                    ...
                    <View style={[styles.editView]}>
                        <TextInput placeholder={user.program} style={[styles.text]} placeholderTextColor={'#FFFFFF'} editable={false} />
                        <TouchableOpacity></TouchableOpacity>
                    </View>
                    <View style={[styles.editView]}>
