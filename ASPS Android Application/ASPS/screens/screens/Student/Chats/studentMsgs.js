...
export default function StudentMsgs({ navigation, route }) {
    const [loading, setLoading] = useState(true)
    const [msgs, setMsgs] = useState([]);
    const [number, setNumber] = useState(0)
    useEffect(() => {
        firebase.database().ref('Chat/' + route.params.chat.studentId + '/' + route.params.chat.teacherid + '/msgs/').on('value', (snapshot) => {
            if (snapshot.val() != undefined && snapshot.val() != null) {
                setMsgs(snapshot.val())
                setNumber(snapshot.val().length)
                setLoading(false)
                if(msgs.length >2 &&  flatListRef.current != null){
    ...
    const sendMsg = () => {
        if (msg != undefined && msg != null) {
            firebase.database().ref('Chat/' + route.params.chat.studentId + '/' + route.params.chat.teacherid + '/msgs/' + number).set({
                sender: route.params.chat.studentId,
                text: msg,
    ...
    const renderMsgs = (item, index) => {
        if (item.sender == route.params.chat.studentId) {
            return (
                <View style={{ alignSelf: 'flex-end', marginVertical: hp('0.5'), borderRadius: hp('2'), backgroundColor: '#767676', paddingHorizontal: wp('4'), paddingVertical: hp('1') }}>
                    <Text style={[{ textAlign: 'left', fontSize: hp('2.5'), color: '#00000090' }]}>{item.text}</Text>
                    <Text style={[{ textAlign: 'right', color: '#FFFFFF90' }]}>{String(converTime(item.time)).substring(4, 25)}</Text>
                </View>
            )
        } else {
            return (
                <View style={{ alignSelf: 'flex-start', marginVertical: hp('0.5'), borderRadius: hp('2'), backgroundColor: '#222222', paddingHorizontal: wp('4'), paddingVertical: hp('1') }}>
                    <Text style={[{ textAlign: 'left', fontSize: hp('2.5'), color: '#FFFFFF' }]}>{item.text}</Text>
                    <Text style={[{ textAlign: 'right', color: '#FFFFFF90' }]}>{String(converTime(item.time)).substring(4, 25)}</Text>
                </View>
            )
        }
        ...
        return (
            <KeyboardAvoidingView style={[styles.container]}>
                ...
                <View style={{ flex: 2.5, flexDirection: 'row', alignItems: 'center', marginHorizontal: wp('4'), marginVertical: hp('0') }}>
                    ...
                    />
                    <TouchableOpacity style={[styles.subjectButton]} onPress={() => { sendMsg() }}>
